...
def setup(path=None):
    ...
    if config_file.has_option("logging", "log_level"):
        try:
            log_level = config_file.get("logging",
                                        "log_level")
        except ValueError:
            print "Configuration file problem: log_level must be an " \
                  "integer value. (logging.DEBUG, logging.INFO, logging.WARNING, logging.ERROR, or logging.CRITICAL)"
            sys.exit(1)
